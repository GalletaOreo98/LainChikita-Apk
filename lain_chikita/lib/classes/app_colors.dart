import 'dart:ui';

class AppColors {
  Color background = const Color.fromARGB(255, 56, 53, 59);
  Color loveBtn = const Color.fromARGB(255, 248, 187, 208);
  Color loveBar = const Color.fromARGB(255, 248, 187, 208);
  Color loveBtnOpposite = const Color.fromARGB(255, 255, 255, 255);
  Color loveBarOpposite = const Color.fromARGB(255, 255, 255, 255);
  Color primaryText = const Color.fromARGB(255, 255, 255, 255);
  Color secondaryText = const Color.fromARGB(255, 173, 173, 173);
  Color nameLabel = const Color.fromARGB(255, 255, 193, 7);
  Color userInputText = const Color.fromARGB(255, 255, 193, 7);
  Color inventoryListBorders = const Color.fromARGB(255, 0, 0, 0);
  Color informativeText = const Color.fromARGB(179, 255, 255, 255);
  Color errorText = const Color.fromARGB(255, 255, 158, 158);
  Color primaryBtn = const Color.fromARGB(255, 255, 87, 87);
  Color secondaryBtn = const Color.fromARGB(255, 255, 87, 87);  
}